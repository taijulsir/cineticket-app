...
export default function EventPageClient({ event: initialEvent, relatedEvents: initialRelatedEvents, slug }: Props) {
  const [event, setEvent] = useState<EventEntity | null>(initialEvent ?? null);
  const [relatedEvents, setRelatedEvents] = useState<EventEntity[]>(initialRelatedEvents ?? []);
  ...
  // Show skeleton while the client hydrates / fetching seat map
  const [isLoading, setIsLoading] = useState(true);
  const [isPaying, setIsPaying] = useState(false);

  // Build grouped shows from the server-provided event once
  useEffect(() => {
    if (!event) return;
    ...
    (event.shows ?? []).forEach((show) => {
      ...
      const date = show.date ? new Date(show.date).toLocaleDateString() : "";
    ...
    // If there is no show to load, clear the loading skeleton
    if (!firstShow) setIsLoading(false);
  }, [event]);

  // Load seat map when a show is selected
  useEffect(() => {
    let mounted = true;
